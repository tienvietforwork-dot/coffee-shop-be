# app-stats -- Thong ke va dieu chinh san pham (Product Stats)

## Gioi thieu

`app-stats` la mot module admin-tool doc lap trong he thong coffee-shop-be. App nay
...
Buoc 2 -- goi thu endpoint ping cua app-stats bang token vua lay duoc:
...
1. Thiet ke bang du lieu can cho thong ke/dieu chinh san pham (vi du: bang
   `product_stat_notes` luu ghi chu phan tich, canh bao ton kho theo san pham,
   de xuat dieu chinh gia/so luong...). Tao bang nay **thu cong** tren Neon qua
   DBeaver, theo dung quy trinh trong `SETUP_GUIDE.docx` cua team (khong dung
   Hibernate ddl-auto, khong dung migration tool).
2. Viet `@Entity` tuong ung (vi du `ProductStatNote.java`) trong package
   `com.coffeeshop.stats.entity`, anh xa dung ten cot da tao tren Neon.
...
5. Deploy len Render nhu mot Web Service rieng, voi Root Directory = `app-stats`.
