# app-promotions -- Quan ly khuyen mai (Promotions)

## Gioi thieu

`app-promotions` la mot module admin-tool doc lap trong he thong coffee-shop-be. App
nay **khong co dang nhap / bang users cua rieng no** -- no tin tuong hoan toan vao
JWT da duoc phat hanh boi `app-core` (module chinh, so huu Auth/Users). Vi vay:
...
Buoc 2 -- goi thu endpoint ping cua app-promotions bang token vua lay duoc:
...
1. Thiet ke bang du lieu can cho khuyen mai (vi du: bang `promotions`/`vouchers`
   luu ma giam gia, phan tram/gia tri giam, thoi gian hieu luc, dieu kien ap
   dung...). Tao bang nay **thu cong** tren Neon qua DBeaver, theo dung quy trinh
   trong `SETUP_GUIDE.docx` cua team (khong dung Hibernate ddl-auto, khong dung
   migration tool).
2. Viet `@Entity` tuong ung (vi du `Promotion.java`, `Voucher.java`) trong package
   `com.coffeeshop.promotions.entity`, anh xa dung ten cot da tao tren Neon.
...
5. Deploy len Render nhu mot Web Service rieng, voi Root Directory =
   `app-promotions`.
